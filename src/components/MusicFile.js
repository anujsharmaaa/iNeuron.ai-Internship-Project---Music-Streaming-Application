...
function LocalComponent({ music_list, cb }) {
  return (
    <>
      {music_list.map((data, index) => {
        return <>
          ...
          <div className="flex">
            {data.map((data, index) => {
              return <CardSection data={data} onClick={cb} list_name={data.list_name} />;
            })}
          </div></>;
      })}
    </>
